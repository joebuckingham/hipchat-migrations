...
module.exports = function (app, addon) {
  ...
  app.post('/webhook',
    addon.authenticate(),
    function (req, res) {
      var command = req.body.item.message.message.slice(11);
      var sender = '@' + req.body.item.message.from.mention_name;
      var response;
      var i;
      switch (command) {
        case "time":
        var total = migrations.length;
        response = total == 1 ? "There is " + total + " migration pending." : "There are " + total + " migrations pending.";
        for (i = 0; i < total; i++) {
          response += "\n" + migrations[i].user + " "+ migrations[i].request;
        }
        break;
        case "done":
        mentions = "";
        for (i = 0; i < migrations.length; i++) {
          if (mentions.indexOf(migrations[i].user) == -1) {
            mentions += " " + migrations[i].user;
          }
        }
        migrations = [];
        response = "All migrations finished. Cc:" + mentions;
        break;
        default:
        migrations.push({
          request: command,
        ...
        response = sender + " Thanks, I've made a note of that.";
        break;
      }

      hipchat.sendMessage(req.clientInfo, req.identity.roomId, response, {options: true, format: "text", notify: true, color: "green"})
        .then(function (data) {
          res.sendStatus(200);
        });
    }
